...
int main() {
    ...
    case 6:
        ...
        if (index_to_remove >= 1 && index_to_remove <= n) {
            ...
            // Сдвигаем элементы массива, чтобы затереть удаляемый элемент
            ...
            n--; // Уменьшаем количество студентов

            students = (struct student*)realloc(students, n * sizeof(struct student)); // Перевыделяем память
